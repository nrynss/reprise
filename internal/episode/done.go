...
// job. A failed episode can retry into the waiting set, so a transient start
// fault does not strand a playable episode. The guarded move runs first, so
// two taps start one render and the second reports ErrIllegalTransition.
// Nothing else moves an episode here: the render pass advances it from there.
//
...
func (s *Service) RequestRender(ctx context.Context, ownerID, episodeID string) (string, error) {
	...
	record, err := s.Get(ctx, ownerID, episodeID)
	if err != nil {
		return "", err
	}
	if record.State == StateFailed {
		if err := Retry(ctx, s.db, episodeID, StateRendering); err != nil {
			return "", err
		}
		return "", nil
	}
